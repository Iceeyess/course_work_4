...
from src.links_constants import hh_API_vacancies
from src.links_constants import file_name
from src.links_constants import HEADERS


def data_insertion():
    """Функция для взаимодействия консоли и пользователя, для
    поискового запроса из hh.ru"""
    path_ = os.path.join('data', file_name)
    print(f"Уважаемый пользователь, представляем программу запроса "
          f"свежих вакансий через API по {hh_API_vacancies}")
    print("Введите поисковые критерии:")
    vacancy = input(f"Введите интересующую Вас вакансию: ").title()
    # Объект класса city, вводим город, пока не найдем city_id по
    # 'https://api.hh.ru/areas' это константа
    city_id = HH.get_city_id(HH_API_CITIES, input("Введите город, в котором "
                                                  "хотите найти вакансию:"
                                                  " ").replace(
        ' ', '').title())
    while not city_id:
        city = input(f"Извините, условие строгое. Вы должны ввести город, в "
                     f"котором хотите найти вакансии:\n").replace(
            ' ', '').title()
    ...
    top_n_vacancies = int(input("Введите top-N вакансий по критериям отбора. Данные "
                                "должны быть целыми числами.\n"))
    does_show_salary_only = input(
        "Необходимо ли показывать только вакансии с указанной заработной платой? "
        "Введите 'Да' или 'Нет'.\n").title()
